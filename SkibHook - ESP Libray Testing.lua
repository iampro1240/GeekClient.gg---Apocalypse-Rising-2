...
esp.connection = RunService.PreRender:Connect(function(deltatime)
  ...
  if self.Character and services:findfirstchild(self.Character, "HumanoidRootPart") and services:findfirstchild(self.Character, "Humanoid") then
    ...
    cache.iscornerbox, cache.isfullbox, cache.distance, cache.name, cache.healthcount = lib2.flags["Boxes"] and lib2.flags["Box_Type"] == "Corner" and os, lib2.flags["Boxes"] and lib2.flags["Box_Type"] == "Full" and os, round(UI.GUI.CurrentDistance) .. "st", cache.getName.Name .. " (卐" .. cache.getName.DisplayName .. ")", round(cache.humanoid.Health)
  ...
  if cache.root ~= nil and cache.humanoid.Health ~= 0 then
   ...
   do
    ...
    do
        ...
        UI.WeaponText.Size = dim2(0.75, 0 * distancemath - .1, 0, .8 / distancemath / 9 + 48)
        ...
        UI.FlagText.Position = dim2(.0999, 0, cache.humanoid.Health - cache.humanoid.MaxHealth, -.14)
        --dim2(.0999, 0, .14, 0)
   ...
   do
    ...
    UI.HealthBar.Size = dim2(0, 1 + distancemath / (pos.Magnitude - (1/2)), .88, 0)
    --dim2(0, 1 + distancemath / pos.Magnitude, .88, 0)
    UI.HealthBar.Position = dim2(.16, 0 * distancemath - clamp(0, 0, 2.5), .12, 0)
