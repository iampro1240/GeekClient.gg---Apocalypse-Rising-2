...
-- Library
    ...
    function Library:Notify(cfg)
        cfg = {
            ...
            Prefix = cfg.Prefix or "[GeekClient.win] ",
    ...
    function Library:Window(cfg)
        ...
        if cfg.Watermark then
            Watermark = Library:Watermark({
                Text = "GeekClient.win | " .. identifyexecutor() .. " | " .. os.date("%H:%M:%S");
                Visible = true;
                Resizing = true
            })
        end
    ...
    function Library:TextTriggers(text)
        ...
        local Triggers = {
            ['{user}'] = Cheat.User,
            ['{name}'] = ('<font color="rgb(%s, %s, %s)">Cheat</font>'):format(mathfloor(AccentTheme.r * 255), mathfloor(AccentTheme.g * 255), mathfloor(AccentTheme.b * 255)),
            ['{hour}'] = os.date("%H"),
            ['{minute}'] = os.date("%M"),
            ['{second}'] = os.date("%S"),
            ['{ap}'] = os.date("%p"),
    ...
    -- Players
        ...
        local MainSec = PlayersTab:Section({Name = "Main"})  --
            ...
            local function PlayerSelected(plr)
                ...
                local PlayerTbl = Cheat.Players[plr]
            ...
            local function SetStatus(status)
                ...
                local PlayerTbl = Cheat.Players[SelectedPlayer]
            ...
            local function RefreshPlayers()
                ...
                for _,v in Players:GetPlayers() do
                    ...
                    if not Cheat.Players[v.Name] then
                        Cheat.Players[v.Name] = {
                            Status = "None",
                            Player = v
                        }
                    end
        ...
        local InfoSec = PlayersTab:Section({Name = "Info", Side = "Right"}) --
            ...
            InfoSec:Button({Name = "Teleport", Confirm = true, Callback = function()
                if not Controller.Alive then
                    Library:Notify({
                        Text = "You are not alive.",
                        Time = 5,
                        Type = "Warning",
                    })
                ...
                if not LocalRoot then
                    Library:Notify({
                        Text = "Failed to get local root part.",
                        Time = 5,
                        Type = "Warning",
                    })
                ...
                if SelectedPlayer == LocalPlayer.Name then
                    Library:Notify({
                        Text = "You can't teleport to yourself.",
                        Time = 5,
                        Type = "Warning",
                    })
                ...
                local Player = FindFirstChild(Players, SelectedPlayer)
                ...
                LocalRoot.Velocity = Vector3zero
                LocalRoot.CFrame = RootPart.CFrame + Vector3new(0, RootPart.Size.Y, 0)
