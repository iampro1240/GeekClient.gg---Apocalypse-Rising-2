	...
	-- elements 
		...
		function library:window(properties)
			...
			-- theming 
				...
				local watermark = library:watermark({default = os.date('GeekClient.win |  - %b %d %Y - %H:%M:%S')})  

				task.spawn(function()
					while task.wait(1) do 
						watermark.change_text(os.date('GeekClient.win - Beta - %b %d %Y - %H:%M:%S'))
					end 
				end) 
